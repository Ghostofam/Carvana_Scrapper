...
for category_id, category_name in categories:
    ...
    for page_number in [None, 2]:  # None for Page 1, 2 for Page 2
        ...
        if page_number is None:
            url = f"{base_url}{category_name}"  # Page 1 URL
        else:
            url = f"{base_url}{category_name}&page={page_number}"  # Page 2 URL
        
        print(f"Opening URL for {category_id.upper()} (Page {page_number or 1}): {url}")
        ...
        try:
            ...
            for link in links:
                ...
                if href:
                    ...
                    cursor.execute("""
                    INSERT INTO Cars (link, category_id)
                    VALUES (?, ?);
                    """, (full_link, category_id))
                else:
    ...
    sheet = workbook.create_sheet(title=category_id.upper())  # Create a new sheet named after the filter
...
connection.commit()
connection.close()
print("Links stored in the Cars table successfully.")
